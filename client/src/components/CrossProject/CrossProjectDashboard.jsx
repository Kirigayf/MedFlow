...
const CrossProjectDashboard = () => {
  ...
  const handleDelete = async (taskId) => {
    // Меняем текст предупреждения, чтобы пользователи понимали последствия
    if (!window.confirm('Внимание! Это действие навсегда удалит саму мастер-задачу, а также ВСЕ связанные с ней карточки на всех досках. Продолжить?')) {
      return;
    }
    try {
  ...
  const handleOpenTask = (task) => {
    ...
    if (task.linkedBoardId && task.linkedCardId) {
      history.push(`https://delay.miac53.ru/cards/${task.linkedCardId}`);
    } else {
